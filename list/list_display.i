...
# 1 "..\\src\\display\\../timers/bsp_systick.h"



# 5 "..\\src\\display\\../timers/bsp_systick.h"
# 6 "..\\src\\display\\../timers/bsp_systick.h"
# 7 "..\\src\\display\\../timers/bsp_systick.h"
...
static void show(list_update_mode mode)
{
    ...
    int item_str_len = 16 - 1 - current_list->icon_num;
    int row_iter;
    char* indi_spa = malloc(3);

    if (!indi_spa)
    {
        lcd_clear_screen(); lcd_print("NO MEMORY!", 0, 0);
        return;
    }
    ...
    if (old_indicator - old_upper_space != cur_sts.indicator -
        ...
        for (row_iter = 0; row_iter < 2; row_iter++)
        {
            if (dis_start + row_iter >= current_list->row_num)
                lcd_print_line_with_num("", row_iter, 1, 16 - 1);
            else
            {
...
static void infi_check(void)
{
    ...
    exti_config(((uint32_t)0x00010), EXTI_Mode_Interrupt, EXTI_Trigger_Rising, ENABLE);
    exti_config(((uint32_t)0x00004), EXTI_Mode_Interrupt, EXTI_Trigger_Rising, ENABLE);
    exti_config(((uint32_t)0x00001), EXTI_Mode_Interrupt, EXTI_Trigger_Falling, ENABLE);
    ...
    while (1)
    {
        ...
        if (penter)
        {
            exti_config(((uint32_t)0x00010), EXTI_Mode_Interrupt, EXTI_Trigger_Rising, DISABLE);
            exti_config(((uint32_t)0x00004), EXTI_Mode_Interrupt, EXTI_Trigger_Rising, DISABLE);
            exti_config(((uint32_t)0x00001), EXTI_Mode_Interrupt, EXTI_Trigger_Falling, DISABLE);
            exti_config(((uint32_t)0x00008), EXTI_Mode_Interrupt, EXTI_Trigger_Rising, ENABLE);
            cfunc(current_list->rows[cur_sts.indicator].message);
            penter = 0;
            exti_config(((uint32_t)0x00010), EXTI_Mode_Interrupt, EXTI_Trigger_Rising, ENABLE);
            exti_config(((uint32_t)0x00004), EXTI_Mode_Interrupt, EXTI_Trigger_Rising, ENABLE);
            exti_config(((uint32_t)0x00001), EXTI_Mode_Interrupt, EXTI_Trigger_Falling, ENABLE);
            exti_config(((uint32_t)0x00008), EXTI_Mode_Interrupt, EXTI_Trigger_Rising, DISABLE);
            pcancel = 0;
            ufunc(current_list);
            show(NORMAL);
        }
        ...
        if (pup)
        {
            ...
            {
                lcd_print_line_with_num("AT TOP!", 0, 0, 16);
                systick_delay(1000);
                show(NORMAL);
            }
        ...
        if (pdown)
        {
            ...
            {
                lcd_print_line_with_num("AT BOTTOM!", 2 - 1, 0,
                                        16);
                systick_delay(1000);
                show(NORMAL);
            }
